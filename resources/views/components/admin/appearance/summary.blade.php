<section class="admin-surface rounded-2xl border border-slate-200/80 bg-white/78 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/58">
    <div class="grid gap-4 xl:grid-cols-[1.7fr_0.9fr]">
        ...
        <aside class="rounded-2xl border border-slate-200 bg-slate-50/75 p-4 dark:border-slate-800 dark:bg-slate-900/60"><div class="flex items-center justify-between"><h3 class="text-sm font-semibold uppercase tracking-[0.28em] text-slate-400 dark:text-slate-500">Working Rhythm</h3><span class="rounded-full border border-slate-200 bg-white px-3 py-1 text-xs font-semibold text-slate-500 dark:border-slate-700 dark:bg-slate-950/70 dark:text-slate-300">{{ ucfirst($activeTab) }}</span></div><div class="mt-3 space-y-2.5">@foreach([['1', 'Branding', 'Upload logo, favicon, and site identity first.'], ['2', 'Homepage', 'Tune trust copy, CTA text, and featured experience.'], ['3', 'Payments', 'Enable only the checkout methods you truly operate.']] as [$step, $title, $text])<div class="flex gap-3 rounded-2xl border border-slate-200 bg-white/80 p-3 dark:border-slate-800 dark:bg-slate-950/60"><div class="flex h-8 w-8 items-center justify-center rounded-xl bg-slate-900 text-xs font-black text-white dark:bg-white dark:text-slate-900">{{ $step }}</div><div><p class="font-semibold text-slate-900 dark:text-white">{{ $title }}</p><p class="mt-1 text-sm leading-6 text-slate-500 dark:text-slate-400">{{ $text }}</p></div></div>@endforeach</div></aside>
    </div>
</section>
<section class="admin-surface rounded-2xl border border-slate-200/80 bg-white/78 p-4 shadow-sm dark:border-slate-800 dark:bg-slate-950/58">
    <div class="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div>
            ...
            <h3 class="mt-1.5 text-xl font-black text-slate-900 dark:text-white">Homepage composition preview</h3>
            <p class="mt-1.5 max-w-3xl text-sm leading-6 text-slate-500 dark:text-slate-400">This preview follows your current appearance settings so you can tune layout, sections, and promo rhythm before saving.</p>
        </div>
    ...
    <div class="mt-4 rounded-2xl border border-slate-200 bg-[linear-gradient(180deg,#eef2ff_0%,#f8fafc_100%)] p-4 dark:border-slate-800 dark:bg-[linear-gradient(180deg,#0f172a_0%,#111827_100%)]">
        <div class="rounded-[1.5rem] p-5 text-white {{ $hero_surface === 'minimal' ? '' : 'shadow-[0_18px_50px_rgba(15,23,42,0.15)]' }}" style="background:{{ $hero_surface === 'minimal' ? 'transparent' : 'linear-gradient(135deg, ' . $hero_bg_from . ', ' . $hero_bg_to . ')' }}; border:1px solid rgba(255,255,255,0.16);">
        ...
        @if($promo_strip_enabled)
            <div class="mt-4 rounded-[1.5rem] p-4 text-white shadow-[0_14px_40px_rgba(15,23,42,0.14)]" style="background:linear-gradient(135deg, {{ $promo_strip_from }}, {{ $promo_strip_to }})">
        ...
        <div class="mt-4 grid gap-3 lg:grid-cols-3">
            @foreach([$deals_section_title, $featured_section_title, $new_arrivals_section_title] as $sectionTitle)
                <div class="rounded-[1.25rem] border border-slate-200 bg-white/80 p-4 dark:border-slate-700 dark:bg-slate-900/70">
