@props(['config' => []])
@php
    $title = $config['title'] ?? 'Featured Display Modules';
    $subtitle = $config['subtitle'] ?? 'Hand-picked top tier replacement displays and touch assemblies.';
    $limit = (int)($config['limit'] ?? 6);
    $products = \App\Models\Stock::visibleOnStorefront()->take($limit)->get();
@endphp

<section class="my-6 max-w-7xl mx-auto px-4">
    <div class="mb-4 flex items-end justify-between">
        <div>
            <p class="text-[10px] font-black uppercase tracking-[0.3em] text-[var(--primary)] mb-1">Featured</p>
    ...
    <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-3 sm:gap-4">
        @forelse($products as $product)
            @include('storefront.partials.product-card', ['product' => $product, 'badge' => 'Featured'])
        @empty
            <div class="col-span-full py-12 text-center rounded-2xl bg-slate-50 dark:bg-slate-900/50 border border-slate-200/60 dark:border-slate-800">
                ...
                <p class="text-xs font-bold text-slate-400">No featured items available.</p>
            </div>
        @endforelse
    </div>
</section>
